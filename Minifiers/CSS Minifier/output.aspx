		...
		switch (sourceType)
		{
			case "folder":
				...
				if (folder.IsLoaded && folder.IsFolder)
				{
					...
					searchParams.Add(AssetPropertyNames.Label, Comparison.EndsWith, ".css");
	...
	public void ProcessAsset(Asset thisAsset, string includeType)
	{
		...
		else
		{
			try
			{
				...
				{
					Out.WriteLine(Util.MinifyCSS(thisAsset.Show()));
				}
				else
				{
			...
			{
				Out.DebugWriteLine("Failed to process css for {0} ({1}), error {2} in {3}", thisAsset.AssetPath, thisAsset.Id, ex.Message, ex.Source);
				Out.WriteLine("<!-- Failed to process css for {0} ({1}), error {2} in {3} -->", thisAsset.AssetPath, thisAsset.Id, ex.Message, ex.Source);
			}
		}
	}
